...
from areas.views import (
    StateSenateDistrictChooserViewSet,
    StateHouseDistrictChooserViewSet,
)


class StateSenateDistrictAdmin(ModelAdmin):
    model = StateSenateDistrict
    menu_label = "State Senate Districts"
...
class StateHouseDistrictAdmin(ModelAdmin):
    ...
    menu_label = "State House Districts"
    # menu_icon = 'pilcrow'  # change as required
    menu_order = 200  # will put in 3rd place (000 being 1st, 100 2nd)
    add_to_settings_menu = False  # or True to add your model to the Settings sub-menu
    exclude_from_explorer = (
...
def register_state_senate_district_chooser_viewset():
    return StateSenateDistrictChooserViewSet(
        "state_senate_district_chooser", url_prefix="state-senate-district-chooser"
    )


@hooks.register("register_admin_viewset")
def register_state_house_district_chooser_viewset():
    return StateHouseDistrictChooserViewSet(
        "state_house_district_chooser", url_prefix="state-house-district-chooser"
    )
