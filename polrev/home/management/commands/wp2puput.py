...
class Command(LabelCommand):
    ...
    async def import_entry(self, slug, title, content, items, item_node):
        ...
        try:
            ...
            page = await sync_to_async(EntryPage)(
                title=title,
                body = json.dumps([
                    {u'type': u'paragraph', u'value': content},
                ]),
    ...
    async def import_entries(self, items):
        ...
        for item_node in items:
            slug = item_node.find(u'{{{0:s}}}post_name'.format(self.WP_NS)).text
            ...
            if post_type == 'post' and content and title:
                ...
                await self.import_entry(slug, title, content, items, item_node)
    ...
    def _import_image(self, image_url):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                image.flush()
                #return image
                #return ImageFile(open(slug, 'rb'), name=slug)
                return ImageFile(image, name=slug)
